...
extern volatile unsigned short take_led_temp_sample;
...
//--- VARIABLES GLOBALES ---//
#ifdef WITH_BOARD_TEMP
#define SIZEOF_BOARD_TEMP	8
unsigned short board_temp [SIZEOF_BOARD_TEMP];
unsigned short last_board_temp = 0;
unsigned char board_temp_index = 0;
unsigned char new_temp_sample = 0;			//la uso para avisar que se puede usar el filtro
#endif

#define SIZEOF_LED_TEMP	8
unsigned short led_temp [SIZEOF_LED_TEMP];
unsigned short last_led_temp = 0;
unsigned char led_temp_index = 0;
unsigned char new_led_temp_sample = 0;			//la uso para avisar que se puede usar el filtro
...
void UpdateLedTemp(void)
{
	//hago update cada 1 seg
	if (!take_led_temp_sample)
	{
		take_led_temp_sample = 1000;

		if (led_temp_index < SIZEOF_LED_TEMP)
			led_temp_index++;
		else
			led_temp_index = 0;

		led_temp [led_temp_index] = ReadADC1_SameSampleTime(ADC_Channel_8);
		new_led_temp_sample = 1;
	}
}

unsigned short GetLedTemp (void)
{
	unsigned char i;
	unsigned int t = 0;

	if (new_led_temp_sample)
	{
		for (i = 0; i < SIZEOF_LED_TEMP; i++)
		{
			t += led_temp[i];
		}

		last_led_temp = (unsigned short) t >> 3;
		new_led_temp_sample = 0;
	}

	return last_led_temp;
}

void UpdateBoardTemp(void)
{
	//hago update cada 1 seg
	if (!take_temp_sample)
	{
...
unsigned short GetBoardTemp (void)
{
	unsigned char i;
	unsigned int t = 0;

	if (new_temp_sample)	//si tengo nuevo sample, lo calculo
	{
		...
		last_board_temp = (unsigned short) t >> 3;
		new_temp_sample = 0;
	}

	return last_board_temp;		//sino devuelvo el anterior
}

short ConvertBoardTemp (unsigned short t_sample)
{
	short dy = 110 - 30;
	short dt = 0;
	short temp = 0;
